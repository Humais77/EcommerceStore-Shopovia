...
const NerArrivals = () => {
    ...
    useEffect(() => {
        const fetchNewArrivals = async () => {
            const storedData = sessionStorage.getItem("newArrivals");

            if (storedData) {
                setNewArrivals(JSON.parse(storedData));
            } else {
                try {
                    ...
                    sessionStorage.setItem("newArrivals", JSON.stringify(selected));
                } catch (error) {
        ...
        <section className='py-16 px-4 lg:px-0'>
            <div className='container mx-auto text-center mb-10 relative'>
                ...
                <p className='text-lg text-gray-600 mb-8'>
                    Discover the latest styles straight off the runway, feshly added to keep your wardrobe on the cutting edge of fashion
                </p>
                ...
                {newArrivals.map((product) => (
                    <div key={product._id} className='min-w-[100%] sm:min-w-[50%] lg:min-w-[30%] relative'>
                        <img src={product.images[0]?.url} alt={product.images[0]?.alttext || product.name}
                            draggable="false" className='w-full h-[500px] object-cover rounded-lg' />
                        <div className='absolute bottom-0 left-0 right-0 bg-opacity-50 backdrop-blur-md text-white p-4 rounded-b-lg '>
                            <Link to={`/products/${product._id}`} className="block ">
                                <h4 className='font-medium text-black'>{product.name}</h4>
                                <p className='mt-1 text-black '>Rs {product.price}</p>
                            </Link>
                        </div>
                    </div>
                ))}
