    ...
    useEffect(() => {
        if (user) {
            if (cart?.products.length > 0 && guestId) {
                dispatch(mergeCart({ email, password })).then(() => {
            ...
            <div className='w-full md:w-1/2 flex flex-col justify-center items-center p-8 md:p-12'>
                <form onSubmit={handleSubmit} className='w-full max-w-md bg-white p-8 rounded-lg border shadow-sm'>
                    <div className='flex justify-center mb-6'>
                        <h2 className='text-xl font-medium'>Shopovia</h2>
                    </div>
                    ...
                    <button className='w-full bg-black text-white p-2 rounded-lg font-semibold hover:bg-gray-800 transition' type='submit'>{loading ? "Loading..." :"Sign Up"}</button>
                    <p className='mt-6 text-center text-sm'>
                        Don't have an account?
                        <Link to={`/register?redirect=${encodeURIComponent(redirect)}`} className='text-blue-500'> Login</Link>
                    </p>
